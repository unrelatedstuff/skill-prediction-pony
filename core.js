...
module.exports = function SPCore(mod) {
	...
	mod.hook('S_MOUNT_VEHICLE_EX', 1, event => { if(isMe(event.target)) vehicleEx = event.vehicle })
	mod.hook('S_UNMOUNT_VEHICLE_EX', 1, event => { if(isMe(event.target)) vehicleEx = null })
	...
	function handleStartSkill(type, event, info, data) {
		...
		if(player.hasNocTan && !nocTanWarning) {
			console.log('[SkillPrediction] Noctenium Infusion detected in inventory. Please bank or discard it before using skills.')console.log(`[SkillPrediction] Noctenium Infusion detected in inventory.
...
(Server bug: Having Noctenium Infusion in inventory causes skill delays)`)
	...
	if(mod.settings.debug.skills.projectiles) {
		...
		mod.hook('C_HIT_USER_PROJECTILE', 4, event => {
			...
			if(skill) {
				...
				if(event.targets.length === 1 && isMe(event.targets[0].gameId)) return false
	...
	function addProjectile(opts) {
		...
		const id = new Long(clientProjectileId = clientProjectileId + 1 >>> 0, 0xfffffff, true);
		...
		if(info.type === 'userProjectile') {
			...
			mod.toClient('S_START_USER_PROJECTILE', 9, {
				...
				projectileSpeed: info.flyingSpeed
			})
		}
	}
	...
	function isMe(id) { return player.gameId.equals(id) || vehicleEx && vehicleEx.equals(id) }
